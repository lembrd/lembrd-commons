...
public class PrimitiveBits {
	public static final byte[] POINT = PrimitiveBits.toBytes(".");
	public static final byte[] X00 = new byte[2];
...
//////////////
    ...
    public static byte[] charSequenceToBytes(CharSequence str) {
		...
		charSequenceToBytes(str, array, 0);
		return array;
    }

    public static byte[] prefixed(byte[] bin) {
        int length = bin.length;
        byte[] array = new byte[length + 2];

        PrimitiveBits.putShort(array, 0, (short) length);
        PrimitiveBits.putBytes(array, 2, bin);
        return array;
    }

/*
    public static byte[] bytesHashed( byte[] bin ) {
        int length = bin.length;
        byte[] array = new byte[length + 4];

        int hash = MurmurHash.hash(bin);

        int pos = PrimitiveBits.putInt(array, 0, hash);

        PrimitiveBits.putBytes(array, pos, bin);

        return array;
    }

    public static byte[] longWithHash( long l ) {
        byte[] array = new byte[8 + 4];

        PrimitiveBits.putLong(array, 4, l);
        int hash = MurmurHash.hash(array, 4, 8, -1);
        PrimitiveBits.putInt(array,0, hash);

        return array;
    }

    public static byte[] strWithHash( String s ) {
        int hash = MurmurHash.hash(s);
        int length = findUTFLength(s);

        byte[] array = new byte[length + 4];

        int pos = PrimitiveBits.putInt(array, 0, hash);

        charSequenceToBytes(s,array, pos);

        return array;
    }
*/

	public static int charSequenceToBytes(CharSequence str, byte[] array, int off) {
		int strlen = str.length();
		int c;
		int i;
